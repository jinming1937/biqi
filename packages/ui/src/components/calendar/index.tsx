...
> {
	...
	getMonthGroup(num = 1, date = new Date()) {
		...
		while (num--) {
			monthArray.push({
				...
				date:
					cacheToDay.getFullYear() +
					'-' +
					(cacheToDay.getMonth() + 1 + '').replace(/^(\d)$/, '0$1'),
				today: date,
			})
			cacheToDay.setDate(
				// must be setDate
				getLastDateFromMonth(cacheToDay.getFullYear(), cacheToDay.getMonth()) +
					1 // the first day of next month
			)
		}
		return monthArray
	}
	...
	getDayGroup(date = new Date()) {
		...
		const dateArray = []
		const count =
			lastDay === 28 && weekOfFirstDay === 0
				? 28
				: weekOfFirstDay >= 5 && lastDay > 29
				? 42
				: 35
		for (let t = 0; t < count; t++) {
			...
			dateArray.push({
				date:
					...
					'-' +
					(beginDate.getMonth() + 1 + '').replace(/^(\d)$/, '0$1') +
					'-' +
					(beginDate.getDate() + '').replace(/^(\d)$/, '0$1'),
				day: beginDate.getDate(),
				today: beginDate, ///// only one
				dayState:
					+beginDate - +firstDayOfMonth < 0 || +beginDate - +lastDayOfMonth > 0,
			})
		}
		return dateArray
	}
									...
									{item.months.map((it: IDates, ind) => {
										return (
											<li
												key={ind}
												className={
													+it.today - +item.today < 0
														? getPrefixCls("disabled")
														: it.date === this.state.defDate && !it.dayState
														? getPrefixCls("active")
														: ''
												}
											>
